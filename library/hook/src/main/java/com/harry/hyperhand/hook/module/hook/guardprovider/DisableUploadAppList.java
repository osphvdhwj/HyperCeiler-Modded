...
*/
package com.harry.hyperhand.hook.module.hook.guardprovider;
...
public class DisableUploadAppList extends BaseHook {
   ...
   /* @Override
    public void handleLoadPackage(final XC_LoadPackage.LoadPackageParam lpparam) {
        if (lpparam.packageName.equals("com.miui.guardprovider")) {
            XposedBridge.log("HyperHand: Start to hook package " + lpparam.packageName);
            ...
            if (guardApplication != null) {
                ...
                for (Field field : guardApplicationFields) {
                    if (field.getName().equals("c")) {
                        XposedHelpers.setStaticBooleanField(guardApplication, "c", true);
                        XposedBridge.log("HyperHand: Info: GuardProvider will work as debug mode!");
                    }
                    XposedBridge.log("HyperHand: Warning: GuardProvider debug mode flag not found!");
                }
            } else {
                XposedBridge.log("HyperHand: Warning: GuardApplication class not found. GuardProvider will not work as debug mode! ");
            }

            // Prevent miui from uploading app list
            final Class<?> antiDefraudAppManager = XposedHelpers.findClassIfExists("com.miui.guardprovider.engine.mi.antidefraud.AntiDefraudAppManager", lpparam.classLoader);
            if (antiDefraudAppManager == null) {
                XposedBridge.log("HyperHand: Skip: AntiDefraudAppManager class not found.");
                return;
            } else {
                XposedBridge.log("HyperHand: Info: AntiDefraudAppManager class found.");
            }
            ...
            if (getAllUnSystemAppsStatus == null) {
                XposedBridge.log("HyperHand: Skip: getAllUnSystemAppsStatus method not found.");
                return;
            } else {
                XposedBridge.log("HyperHand: Info: getAllUnSystemAppsStatus method found.");
            }
            ...
            XposedBridge.hookMethod(getAllUnSystemAppsStatus, new XC_MethodHook() {
                ...
                protected void beforeHookedMethod(MethodHookParam methodHookParam) throws Throwable {
                    ...
                    if (MIUI_VERSION == null) {
                        XposedBridge.log("HyperHand: Warning: Can't get MIUI_VERSION.");
                    }
                    ...
                    if (uuidHelper != null) {
                        ...
                        if (getUUID != null) {
                            ...
                            XposedBridge.log("HyperHand: Warning: getUUID method not found.");
                        }
                    } else {
                        XposedBridge.log("HyperHand: Warning: uuidHelper class not found.");
                    }
                    ...
                    XposedBridge.log("HyperHand: Info: Intercept=" + jSONObject.toString());
