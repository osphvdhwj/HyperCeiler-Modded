...
public class AllowUninstall implements IXposedHookZygoteInit {
    ...
    public void initZygote(StartupParam startupParam) {
        // XposedBridge.log("[HyperHand][I][android][AllowUninstall]: hook all PathClassLoader Constructors");
        pathClassLoaderHook =
            XposedBridge.hookAllConstructors(PathClassLoader.class, new XC_MethodHook() {
                ...
                protected void afterHookedMethod(MethodHookParam param) {
                    ...
                    if (path.contains("/system/framework/services.jar")) {
                        // XposedBridge.log("[HyperHand][I][android][AllowUninstall]: find services.jar ClassLoader");
                        try {
                            ...
                            SecurityManagerServiceClazz = XposedHelpers.findClass(
                                ...
                                servicesClassLoader);
                            // XposedBridge.log("[HyperHand][I][android][AllowUninstall]: findClass SecurityManagerService$1");
                            XposedHelpers.findAndHookMethod(SecurityManagerServiceClazz,
                                "run", new XC_MethodReplacement() {
                                    ...
                                    protected Object replaceHookedMethod(MethodHookParam unused) {
                                        // XposedBridge.log("[HyperHand][I][android][AllowUninstall]: hooked checkSystemSelfProtection invoke");
                                        return null;
                                    }
                                });
                            // XposedBridge.log("[HyperHand][I][android][AllowUninstall]: hook method 'SecurityManagerService$1.run()'");
                        } catch (Exception e) {
                            XposedBridge.log("[HyperHand][E][android][AllowUninstall]: AllowUninstall Exception! " + e);
                            // e.printStackTrace();
                        } finally {
                            for (Unhook hook : pathClassLoaderHook) {
                                hook.unhook();
                            }
                            // XposedBridge.log("[HyperHand][I][android][AllowUninstall]: unhook all PathClassLoader Constructors");
                        }
                    }
                }
            });
