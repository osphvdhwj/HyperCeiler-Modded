...
public class CorePatchForR extends XposedHelper implements IXposedHookLoadPackage {
    ...
    static {
        ...
        try {
            ...
            XposedBridge.log("[HyperHand][E][android]" + TAG + ": " + Log.getStackTraceString(t));
        }
        deoptimizeMethod = m;
    }
        ...
        for (Method m : c.getDeclaredMethods()) {
            if (deoptimizeMethod != null && m.getName().equals(n)) {
                ...
                if (isNotReleaseVersion)
                    XposedBridge.log("[HyperHand][D][android]" + TAG + ": Deoptimized " + m.getName());
            }
        }
    }
    ...
    public void handleLoadPackage(XC_LoadPackage.LoadPackageParam loadPackageParam) throws IllegalAccessException, InvocationTargetException, InstantiationException {
        if (isNotReleaseVersion) {
            XposedBridge.log("[HyperHand][D][android]" + TAG + ": downgrade=" + prefs.getBoolean("prefs_key_system_framework_core_patch_downgr", true));
            XposedBridge.log("[HyperHand][D][android]" + TAG + ": authcreak=" + prefs.getBoolean("prefs_key_system_framework_core_patch_auth_creak", true));
            XposedBridge.log("[HyperHand][D][android]" + TAG + ": digestCreak=" + prefs.getBoolean("prefs_key_system_framework_core_patch_digest_creak", true));
            XposedBridge.log("[HyperHand][D][android]" + TAG + ": UsePreSig=" + prefs.getBoolean("prefs_key_system_framework_core_patch_use_pre_signature", false));
            XposedBridge.log("[HyperHand][D][android]" + TAG + " sharedUser=" + prefs.getBoolean("prefs_key_system_framework_core_patch_shared_user", false));
            XposedBridge.log("[HyperHand][D][android]" + TAG + "disableVerificationAgent=" + prefs.getBoolean("prefs_key_system_framework_disable_verification_agent", true));
        }
                    ...
                    if (throwable != null || parseErr != null) {
                        ...
                        try {
                            if (prefs.getBoolean("prefs_key_system_framework_core_patch_use_pre_signature", false)) {
                                ...
                                if (PM == null) {
                                    XposedBridge.log("[HyperHand][E][android]" + TAG + ": [" + ProjectApi.mAppModulePkg + "] Cannot get the Package Manager... Are you using MiUI?");
                                } else {
                                    PackageInfo pI;
                                    if (parseErr != null) {
        ...
        if (utilClass != null) {
            try {
                ...
                XposedBridge.log("[HyperHand][E][android]" + TAG + ": deoptimize failed" + Log.getStackTraceString(e));
            }
        }
        ...
        XposedBridge.hookAllMethods(sharedUserSettingClass, "addPackage", new XC_MethodHook() {
                ...
                protected void beforeHookedMethod(MethodHookParam param) throws Throwable {
                    ...
                    XposedBridge.log("[HyperHand][D][android]" + TAG + ": updating signature in sharedUser during add " + toAdd + ": " + param.thisObject);
                    Setting_setSigningDetails(param.thisObject, newSig);
                }
            }
        );
